...
export class UserService {
  ...
  Register(model: UserModel): Observable<UserModel> {
    return this.http
      .put<UserModel>(`${environment.ApiURL}/user/register`, model)
      .pipe(
        map((data: UserModel) => {
          return data;
        }),
  ...
  saveImage(image: any, userId: number): Observable<ImagesModel> {
    return this.http
      .put<ImagesModel>(
        `${environment.ApiURL}/user/image?userId=${userId}`,
        image
      )
      .pipe(
  ...
  getImageById(image_id: any): Observable<ImagesModel> {
    return this.http
      .get<ImagesModel>(`${environment.ApiURL}/user/image?image_id=${image_id}`)
      .pipe(
        map((data: any) => {
          return data;
        }),
  ...
  getTrainer(training_id: any): Observable<UserModel> {
    return this.http
      .get<UserModel>(
        `${environment.ApiURL}/user/getTrainer?training_id=${training_id}`
      )
      .pipe(
        map((data: any) => {
          return data;
  ...
  emailExists(email: string): Observable<boolean> {
    return this.http
      .get<UserModel>(`${environment.ApiURL}/user/email?email=${email}`)
      .pipe(
        map((data: any) => {
          return data;
        }),
  ...
  Login(email, password): Observable<UserModel> {
    return this.http
      .get<UserModel>(
        `${environment.ApiURL}/user/login?email=${email}&password=${password}`
      )
      .pipe(
        map((data: UserModel) => {
          return data;
  ...
  modifyUser(model: UserModel): Observable<UserModel> {
    return this.http
      .post<UserModel>(`${environment.ApiURL}/user/modify`, model)
      .pipe(
        map((data: UserModel) => {
          return data;
        }),
  ...
  deleteUser(model: UserModel): Observable<any> {
    ...
    return this.http.delete<any>(`${environment.ApiURL}/user`, options).pipe(
      map((data: any) => {
        return data;
      }),
      catchError((err) => {
